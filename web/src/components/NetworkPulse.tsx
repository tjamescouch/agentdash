    ...
    for (const channel of channels) {
      ...
      if (existing) {
        ...
        nodes.set(channel.name, {
          id: channel.name, label: channel.name, type: 'channel',
  ...
  useEffect(() => {
    const tick = () => {
      ...
      for (const edge of edges) {
        ...
        const fx = dx * ATTRACTION; const fy = dy * ATTRACTION;
      ...
      for (const p of particlesRef.current) {
        ...
        ctx.fillStyle = p.color.replace(')', `, ${alpha})`).replace('hsl', 'hsla'); ctx.fill();
        ctx.beginPath(); ctx.arc(x, y, 6, 0, Math.PI * 2);
        ctx.fillStyle = p.color.replace(')', `, ${alpha * 0.3})`).replace('hsl', 'hsla'); ctx.fill();
      }
      ...
      for (const node of nodes) {
        ...
        if (node.type === 'channel') {
          ...
          ctx.strokeStyle = 'rgba(0, 191, 255, 0.6)'; ctx.lineWidth = 1.5;
          ctx.fill(); ctx.stroke();
        } else {
          ...
          ctx.lineWidth = node.verified ? 2 : 1; ctx.fill(); ctx.stroke();
          if (node.verified) {
            ctx.beginPath(); ctx.arc(node.x, node.y, node.radius + 2, 0, Math.PI * 2);
            ctx.strokeStyle = 'rgba(0, 191, 255, 0.5)'; ctx.lineWidth = 1; ctx.stroke();
          }
        }
      ...
      if (hoveredRef.current) {
        ...
        if (node) {
          ...
          const padding = 8; const lineHeight = 14;
          ...
          ctx.beginPath(); ctx.roundRect(tx, ty, tooltipW, tooltipH, 4); ctx.fill(); ctx.stroke();
